...
class CtdbRootDirService(Service):
    ...
    def init(self, job, gvol_name):
        ...
        We use the native gluster file I/O API via the py-glfs module. This means
        ...
        try:
            ...
            if e.errno != errno.EEXIST:
                raise CallError(f'Failed to create {ROOT_DIR-NAME!r}')

        # set perms
        dir_fd = root_handle.lookup(path).open(os.O_DIRECTORY)
        stat = dir_fd.fstat()
